...
; DESCRIPTOR64 is used for full-memory minidumps where all of the raw memory
; is laid out sequentially at the end of the dump.  There is no need for
; individual RVAs as the RVA is the base RVA plus the sum of the preceeding
; data blocks.
...
; A normal minidump contains just the information necessary to capture stack traces for all of the existing threads in a process.
;
; A minidump with data segments includes all of the data sections from loaded modules in order to capture global variable contents.  This can make the dump much larger if many modules have global data.
;
; A minidump with full memory includes all of the accessible memory in the process and can be very large.  A minidump with full memory always has the raw memory data at the end of the dump so that the initial structures in the dump can be mapped directly without having to include the raw memory information.
;
; Stack and backing store memory can be filtered to remove data unnecessary for stack walking.  This can improve compression of stacks and also deletes data that may be private and should not be stored in a dump. Memory can also be scanned to see what modules are referenced by stack and backing store memory to allow omission of other modules to reduce dump size. In either of these modes the ModuleReferencedByMemory flag is set for all modules referenced before the base module callbacks occur.
;
; On some operating systems a list of modules that were recently unloaded is kept in addition to the currently loaded module list.  This information can be saved in the dump if desired.
;
; Stack and backing store memory can be scanned for referenced pages in order to pick up data referenced by locals or other stack memory.  This can increase the size of a dump significantly.
;
; Module paths may contain undesired information such as user names or other important directory names so they can be stripped. This option reduces the ability to locate the proper image later and should only be used in certain situations.
;
...
;
; Code which is concerned with potentially private information getting into the minidump can set a flag that automatically modifies all existing and future flags to avoid placing unnecessary data in the dump.  Basic data, such as stack information, will still be included but optional data, such as indirect memory, will not.
;
; When doing a full memory dump it's possible to store all of the enumerated memory region descriptive information in a memory information stream.
;
; Additional thread information beyond the basic thread structure can be collected if desired.
;
; A minidump with code segments includes all of the code and code-related sections from loaded modules in order to capture executable content.
;
; MiniDumpWithoutAuxiliaryState turns off any secondary, auxiliary-supported memory gathering.
;
; MiniDumpWithFullAuxiliaryState asks any present auxiliary data providers to include all of their state in the dump. The exact set of what is provided depends on the auxiliary. This can be quite large.
;
; MiniDumpFilterWriteCombinedMemory asks to exclude all memory with the virtual protection attribute of PAGE_WRITECOMBINE.
...
; In addition to the primary flags provided to MiniDumpWriteDump there are additional, less frequently used options queried via the secondary flags callback.
;
; MiniSecondaryWithoutPowerInfo suppresses the minidump query that retrieves processor power information for MINIDUMP_MISC_INFO.
...
; The minidump callback should modify the FieldsToWrite parameter to reflect what
; portions of the specified thread or module should be written to the file.
;
; BOOL (WINAPI * MINIDUMP_CALLBACK_ROUTINE) (
...
struct MINIDUMP_CALLBACK_INFORMATION
	CallbackRoutine	dq ? ; MINIDUMP_CALLBACK_ROUTINE
	CallbackParam	dq ?
ends
